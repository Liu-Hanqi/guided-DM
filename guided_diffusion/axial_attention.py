...
from guided_diffusion.reversible import ReversibleSequence
...
class SelfAttention(nn.Module):
    def __init__(self, dim, heads, dim_heads = None):
        ...
        self.heads = heads
        self.to_q = linear(dim, dim_hidden, bias = False).to(torch.float16)
        self.to_kv = linear(dim, 2 * dim_hidden, bias = False).to(torch.float16)
        self.to_out = linear(dim_hidden, dim).to(torch.float16)
...
class AxialAttention(nn.Module):
    def __init__(self, dim, use_checkpoint=False, num_dimensions = 2, heads = 8, dim_heads = None, dim_index = -1, sum_axial_out = True):
    ...
    def _forward(self, x):
        ...
        if self.sum_axial_out:
            return sum(map(lambda axial_attn: axial_attn(self.norm(x)), self.axial_attentions))
